...
import one.digitalinnovation.notescontentprovider.database.NotesDatabaseHelper.Companion.TABLE_NOTES
...
class NotesProvider : ContentProvider() {
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<String>?): Int {
        if (mUriMatcher.match(uri) == NOTES_BY_ID){
            ...
            val linesAffect:Int = db.delete(TABLE_NOTES, "${BaseColumns._ID} =?", arrayOf(uri.lastPathSegment))
            db.close()
    ...
    companion object{
        const val AUTHORITY = "one.digitalinnovation.notascontentprovider.provider"
